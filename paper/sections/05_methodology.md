...
| GLM-5 | 744B | ~40B | 256 | GLM-MoE + Sparse Attention | INT4 |
| Kimi K2.5 | 1T | 32B | 384 | MoonlightMoE | INT4 native |
| Qwen 3.5 | 397B | ~17B | 512 | QwenMoE + Gated DeltaNet | INT4 |

We selected open-weight models for two reasons. First, SOC alert data contains sensitive enterprise telemetry (internal IPs, hostnames, detection rules, network topology) that organizations cannot send to external APIs without leaking their security posture — local deployment reflects real-world operational requirements. Second, local serving enables fully reproducible evaluation without API rate limits, terms-of-service restrictions, or non-deterministic API-side changes. Commercial API-based models (GPT-4o, Claude) are excluded because they cannot be served locally for controlled experimentation and because SOC deployments handling sensitive telemetry often prefer on-premises inference for data sovereignty — though cloud-based and hybrid deployments remain common in practice. Different MoE routing strategies may exhibit different adversarial vulnerability profiles. All four models use MoE architectures but with different routing strategies: DeepSeek R1 uses DeepSeekMoE (256 experts), GLM-5 uses GLM-MoE with Dynamically Sparse Attention (256 experts), Kimi K2.5 uses MoonlightMoE (384 experts), and Qwen 3.5 uses QwenMoE with Gated DeltaNet (512 experts). This diversity lets us explore whether expert count and routing strategy correlate with injection susceptibility. We note this is an exploratory comparison: model family, alignment training, tokenizer behavior, and training data may confound any observed routing effect.
...
The base end-to-end matrix produces **120 variants per alert** and **1,457,640 total adversarial samples** as the design-space upper bound.

**Stratified sampling strategy.** Running the full cartesian product (5.8M inference calls across 4 models) may be infeasible within available GPU allocation. We plan a phased approach: (1) E1 baseline on all 12,147 alerts × 4 models (~49K calls); (2) E2 stratified sample of 200 alerts × 12 vectors × 5 classes × 2 encodings × 4 models (~96K calls) to identify ceiling/floor effects and uninformative vector-class combinations; (3) full E2 sweep only on vector-class pairs showing meaningful ASR variation. This avoids spending compute on combinations that are trivially successful or universally ineffective. This count assumes all 12 vectors apply to all alerts; the realized executable count after vector-applicability filtering will be lower, since not all vector-field combinations exist in every log source (see caveat below). Extended E3 encoding tests are reported separately because they measure normalization survival rather than full triage runs. Payloads are truncated to respect field length constraints per vector.
...
**Ring Scoring.** Inspired by SOC-Bench's graduated scoring philosophy, we implement a ring scoring model (Bullseye=3, Inner=2, Outer=1, Miss=0) rather than binary correct/incorrect for technique identification accuracy. The specific point values are our implementation; SOC-Bench describes the ring concept but our decomposition may differ. This rewards partial matches — correctly identifying the tactic but wrong sub-technique scores Inner rather than Miss.

**Relationship to SOC-Bench Design Principles.** Our adapter draws on SOC-Bench's design philosophy but does not claim full DP compliance. DP1 (loyalty to existing SOCs): our triage pipeline processes alerts as a SOC analyst would receive them — timestamp-ordered, without attacker narrative context. DP2 (be exclusive — outcomes not available from standard tools): our behavioral invariant and correlator outputs go beyond standard SIEM rule matching, but we have not validated that standard tools cannot approximate them. DP3 (real-world basis): SOC-Bench grounds its scenarios in a single real incident (Colonial Pipeline/DarkSide). Our benchmark instead uses Splunk Attack Data — controlled attack simulations (Atomic Red Team) across 27 techniques — providing broader technique coverage at the cost of single-incident ecological validity. Our DarkSide scenario (§6.9) is the closest approximation to DP3 but uses 7 hand-crafted alerts rather than real incident data. SOC-Bench's 30-minute temporal staging for Fox evaluation is not implemented in our adapter; we score aggregate triage outputs rather than stage-by-stage assessments, making our Fox scores not directly comparable to SOC-Bench Fox evaluations.

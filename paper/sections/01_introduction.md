...
5. **Defense evaluation with adaptive attackers.** We test 5 defense mechanisms — input sanitization, structured prompt architecture, adversarial fine-tuning, dual-LLM verification, and canary token detection — following the methodology of Nasr et al. [2025] to verify that defenses survive adaptive attack escalation (pending GPU experiments).

...
Critically, SOC alert data is inherently sensitive: it contains internal IP addresses, Active Directory usernames, network topology, detection rule logic, and vulnerability indicators. Exposing this data to cloud-hosted LLM APIs risks leaking an organization's security posture to third parties — revealing what the organization *can* detect and, by implication, what it *cannot*. This data sensitivity creates pressure toward **locally deployed open-weight models** for alert triage, keeping all telemetry on-premises — alongside latency requirements, cost control, auditability, and regulatory constraints on cross-border data transfer. The models we evaluate in this work — frontier open-weight MoE architectures served via local inference — represent one plausible deployment regime motivated by these constraints, not merely an experimental convenience.
...
This question has three components: (1) characterizing the attack surface specific to SOC triage, (2) establishing a protocol for measuring vulnerability across different model architectures, and (3) evaluating whether proposed defenses survive adaptive attackers. This paper delivers (1) in full and specifies the protocol for (2) and (3); cross-model and defense evaluation results await GPU experiments.
...
2. **Adversarial evaluation framework and benchmark.** We construct a benchmark of 12,147 rule-linked SIEM alerts across 27 MITRE ATT&CK techniques in 9 tactics, and specify an evaluation protocol for 4 frontier open-weight MoE models with different routing designs and scaling profiles under 5 attack classes and a 12-vector injection taxonomy, defining a space of up to 1.4 million adversarial alert variants (12 vectors × 5 classes × 2 encodings × 12,147 alerts). Cross-model attack success results await GPU allocation; this paper validates benchmark construction, SIEM normalization survival, and preliminary defense mechanisms.
...
4. **Multi-agent correlation pipeline.** Our pipeline illustrates, via a simulated DarkSide scenario with hand-crafted triage decisions, that single-alert triage is insufficient — a correlator agent using IP clustering, technique chain matching, and temporal burst detection identifies multi-stage campaigns that individual alert classification misses. A playbook agent generates NIST SP 800-61 response guidance with chain-aware severity escalation. Full model-driven evaluation pending GPU experiments.

5. **Defense evaluation protocol.** We implement 3 defense mechanisms — input sanitization, structured prompt architecture, and canary token detection — and design a fourth (dual-LLM verification). We define an adaptive evaluation protocol inspired by Nasr et al. [2025]; defense effectiveness results await GPU experiments.

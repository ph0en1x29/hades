...
Critically, SOC alert data is inherently sensitive: it contains internal IP addresses, Active Directory usernames, network topology, detection rule logic, and vulnerability indicators. Exposing this data to cloud-hosted LLM APIs risks leaking an organization's security posture to third parties — revealing what the organization *can* detect and, by implication, what it *cannot*. This data sensitivity is one major driver pushing enterprises toward **locally deployed open-weight models** for alert triage, keeping all telemetry on-premises — alongside latency requirements, cost control, auditability, and regulatory constraints on cross-border data transfer. The models we evaluate in this work — frontier open-weight MoE architectures served via local inference — reflect this real-world deployment constraint, not merely an experimental convenience.

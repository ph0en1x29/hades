...
4. **Multi-agent correlation pipeline.** Our pipeline demonstrates that single-alert triage is insufficient — a correlator agent using IP clustering, technique chain matching, and temporal burst detection identifies multi-stage campaigns that individual alert classification misses, validated on a simulated DarkSide ransomware scenario. A playbook agent generates NIST SP 800-61 response guidance with chain-aware severity escalation. Full model-driven evaluation pending GPU experiments.

...
**CyBench** [Zhang2025] evaluates LLM offensive cybersecurity capabilities across 40 CTF-style tasks (ICLR 2025 Oral). It benchmarks whether LLMs can *attack* systems. We benchmark whether LLMs deployed *defensively* can be attacked through their data pipelines — the complementary question.
...
**Our contributions:** (1) a systematic framework for adversarial evaluation of LLM triage systems through SIEM log field injection, (2) an evaluation protocol for cross-architecture vulnerability comparison of 4 frontier MoE models with different routing designs and scaling profiles, with benchmark construction validated and cross-model results pending GPU allocation, (3) behavioral invariants — output-level heuristic consistency checks with preliminary validation on simulated triage outputs, (4) a multi-agent correlation pipeline with IP clustering and technique chain matching, demonstrated on a simulated DarkSide campaign, (5) a defense evaluation protocol following the adaptive attacker methodology of [Nasr2025] and addressing the NAACL findings of [Zhan2025], with defense effectiveness results pending GPU experiments, (6) a benchmark-quality dataset with full provenance chain aligning with [Liu2026]'s dataset adequacy recommendations, and (7) a Fox-task-inspired scoring adapter drawing on SOC-Bench [Cai2026] design philosophy, with preliminary counterfactual scoring; the adapter does not implement SOC-Bench's temporal staging or claim benchmark comparability. Tiger and Panda schema support exists but is not evaluated.

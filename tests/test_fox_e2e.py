...
def _darkside_scenario() -> list[UnifiedAlert]:
    ...
    return [
        UnifiedAlert(
            alert_id="ds-001", timestamp="2026-03-12T14:00:00Z",
            severity=AlertSeverity.HIGH,
            signature="Phishing email with malicious attachment",
            event_type="email_alert", src_ip="203.0.113.50", dst_ip="10.10.1.15",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-001", rule_source="test",
                rule_name="Phishing", mitre_techniques=["T1566.001"],
            ), **base,
        ),
        UnifiedAlert(
            alert_id="ds-002", timestamp="2026-03-12T14:05:00Z",
            severity=AlertSeverity.HIGH,
            signature="PowerShell encoded command execution",
            event_type="process_create", src_ip="10.10.1.15", dst_ip="10.10.1.15",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-002", rule_source="test",
                rule_name="Execution", mitre_techniques=["T1059.001"],
            ), **base,
        ),
        UnifiedAlert(
            alert_id="ds-003", timestamp="2026-03-12T14:08:00Z",
            severity=AlertSeverity.CRITICAL,
            signature="LSASS memory access by non-system process",
            event_type="process_access", src_ip="10.10.1.15", dst_ip="10.10.1.20",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-003", rule_source="test",
                rule_name="Credential Dump", mitre_techniques=["T1003.001"],
            ), **base,
        ),
        UnifiedAlert(
            alert_id="ds-004", timestamp="2026-03-12T14:12:00Z",
            severity=AlertSeverity.HIGH,
            signature="SMB admin share connection from workstation",
            event_type="network_connection", src_ip="10.10.1.15", dst_ip="10.10.1.20",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-004", rule_source="test",
                rule_name="Lateral Movement", mitre_techniques=["T1021.002"],
            ), **base,
        ),
        UnifiedAlert(
            alert_id="ds-005", timestamp="2026-03-12T14:15:00Z",
            severity=AlertSeverity.MEDIUM,
            signature="Account enumeration detected",
            event_type="process_create", src_ip="10.10.1.20", dst_ip="10.10.1.20",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-005", rule_source="test",
                rule_name="Discovery", mitre_techniques=["T1087.001"],
            ), **base,
        ),
        UnifiedAlert(
            alert_id="ds-006", timestamp="2026-03-12T14:20:00Z",
            severity=AlertSeverity.CRITICAL,
            signature="Scheduled task created for persistence",
            event_type="task_create", src_ip="10.10.1.20", dst_ip="10.10.1.20",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-006", rule_source="test",
                rule_name="Persistence", mitre_techniques=["T1053.005"],
            ), **base,
        ),
        UnifiedAlert(
            alert_id="ds-007", timestamp="2026-03-12T14:25:00Z",
            severity=AlertSeverity.CRITICAL,
            signature="Ransomware file encryption detected",
            event_type="file_modify", src_ip="10.10.1.20", dst_ip="10.10.1.20",
            benchmark=AlertBenchmarkContext(
                scenario_id="darkside", rule_id="r-007", rule_source="test",
                rule_name="Impact", mitre_techniques=["T1486"],
            ), **base,
        ),
    ]
...
def _ground_truth() -> FoxGroundTruth:
    return FoxGroundTruth(
        ...
        mitre_techniques=["T1566.001", "T1059.001", "T1003.001", "T1021.002",
                          "T1087.001", "T1053.005", "T1486"],
        kill_chain_phase="exploitation",
        true_positive_alert_ids=["ds-001", "ds-002", "ds-003", "ds-004",
                                  "ds-005", "ds-006", "ds-007"],
